...
/*
	Modified by Tuomas Karhu (tuomas.karhu@iki.fi)
*/
...
GDocs.prototype.auth = function(interactive, success_callback, not_auth_callback, fail_callback) {
  try {
    chrome.identity.getAuthToken({interactive: interactive}, function(token) {
    // chrome.experimental.identity.getAuthToken({interactive: interactive}, function(token) {
      ...
      if (token) {
        ...
        success_callback && success_callback();
			...
			// user is not authenticated
				...
				not_auth_callback && not_auth_callback();
			}

    }.bind(this));
  }
	// failed to retrieve authentication info
	catch(e) {
		dbg('***WARNING: GDocs.auth(): failed with error = ' + e);
